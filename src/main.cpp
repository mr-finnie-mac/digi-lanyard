...
gpio_num_t SECOND_BTN_PIN = GPIO_NUM_35;
...
void display_disability(int disabilityType, Language language) {
    const DisabilityInfo* info = getDisplayInfo(disabilityType, language);
    // first set full screen to white
    epd_hl_set_all_white(&hl);

    if (info != NULL) {
        // Display the heading with OpenSans24B
        EpdFontProperties headingFontProps = epd_font_properties_default();
        headingFontProps.flags = EPD_DRAW_ALIGN_LEFT;
        int headingCursorX = 10;
        int headingCursorY = 40;
        epd_write_string(&OpenSans24B, info->heading, &headingCursorX, &headingCursorY, fb, &headingFontProps);

        // Display the paragraph with OpenSans14B
        EpdFontProperties paragraphFontProps = epd_font_properties_default();
        paragraphFontProps.flags = EPD_DRAW_ALIGN_LEFT;
        int paragraphCursorX = 10;
        int paragraphCursorY = headingCursorY + 30;  // Adjust the spacing as needed
        epd_write_string(&OpenSans14B, info->paragraph, &paragraphCursorX, &paragraphCursorY, fb, &paragraphFontProps);
    }
...
// // new stuff
// void display_center_message_with_disability(int disabilityType) {
//     const char* text = getDisplayText(disabilityType);
...
//     epd_write_string(&OpenSans24B, text, &cursor_x, &cursor_y, fb, &font_props);
...
// void display_full_screen_left_aligned_text_with_disability(int disabilityType) {
//     const char* text = getDisplayText(disabilityType);
...
//     int cursor_x = 10;
//     int cursor_y = 30;
//     epd_write_string(&OpenSans14B, text, &cursor_x, &cursor_y, fb, &font_props);
//     /********************************************************/
...
// readButton(){
//       int buttonState = digitalRead(FIRST_BTN_PIN);
//     // If the button i
//     if (buttonState == HIGH) {
//         storedButtonState = 
//     } else {
...
void switchLanguage() {
    ...
    int buttonState = digitalRead(SECOND_BTN_PIN);
    

    // If the button is pressed, move to the next language
    if (buttonState == HIGH) {
        Serial.println("Second button not pressed");
...
void setup() {
    ...
    // Read the state of the button during setup
